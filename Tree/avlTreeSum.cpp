...
struct Node* deleteNode(struct Node* root, int key)
{
    ...
    // Update the sum
    root->sum = root->sum + (root->left!=NULL?root->left->sum:0) + (root->right!=NULL?root->right->sum:0);
...
///calculate prefix sum of a node P(X) = sum of all nodes in DS <= x
int prefixSum(struct Node* root, int x) {
    int sum = 0;
    struct Node* t = root;
    while(t) {
        if(t->key == x)
            return (t->key + (t->left != NULL ? t->left->sum : 0)) + sum;
        else if(t->key < x) {
            sum = sum + t->key + (t->left != NULL ? t->left->sum : 0);
           // cout<<"sum= "<<sum<<endl;
            t = t->right;
        }
        else
...
///sum of all nodes between x and y = P(y) - P(x-1)
int sum(struct Node* root, int x, int y) {
    return prefixSum(root,y) - prefixSum(root,x-1);
}

///average of no.s between x and y
float avg(struct Node* root, int x, int y) {
    return (sum(root, x, y) * 1.0) / (num(root,x,y)+ (search(root,y) != NULL? 1 :0));  ///+1 as num(x,y) doesn't includes y
}
...
int main()
{
  ...
  /* Constructing tree given in the above figure */
    ...
    */
    ...
    cout<<"node with rank 7 "<<findRank(root,7)->key<<endl;

    cout<<"\nPrefix sum of 9 is: "<<prefixSum(root,9);
    cout<<"\nSum of all nodes between 2 and 11: "<<sum(root, 2, 11);
    cout<<"\nAverage of all nodes between 2 and 11 is: "<<avg(root,2,11);
